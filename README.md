...
```bash
functional-scala-lambda$ sam build
```
...
```bash
functional-scala-lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
functional-scala-lambda$ sam local start-api
functional-scala-lambda$ curl http://localhost:3000/
```
...
```bash
functional-scala-lambda$ sam logs -n HelloWorldFunction --stack-name functional-scala-lambda --tail
```
...
```bash
functional-scala-lambda$ cd HelloWorldFunction
...
```bash
aws cloudformation delete-stack --stack-name functional-scala-lambda
```
